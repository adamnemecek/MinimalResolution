...
unsigned cofree_comodule<algebroid, degree_type>::findPos(unsigned startPos, unsigned endPos, unsigned n) const
{
	...
	{
		if (position_of_gens[startPos] <= n)
			return startPos;
		else
			return startPos - 1;
	}
	unsigned midPos = (startPos + endPos) / 2;
	if (position_of_gens[midPos] == n)
		return midPos;
	else if (position_of_gens[midPos] > n)
		return findPos(startPos, midPos, n);
	else
		return findPos(midPos + 1, endPos, n);
}
