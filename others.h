virtual vect maps_to(vect_ref x, std::function<vect(index)> rows)
{
	...
	std::function<vect(vect &&, vect &&)> adding = [this](vect &&x, vect &&y)
					   ...
					   unsigned start, unsigned stop, std::function<modul1(index)> find)
{
	...
	if (stop <= start)
		return modop2->zero();
	if (stop - start == 1)
		return scalor_mult(x.dataArray[start].coeficient, find(x.dataArray[start].ind));
						   ...
						   std::function<modul1(index)> find)
{
	...
	std::function<modul2(modul2 &&, modul2 &&)> adding = [modop2](modul2 &&x, modul2 &&y)
	{ return modop2->add(std::move(x), std::move(y)); };
	if (x.size() == 0)
		return modop2->zero();
...
index1 symplify_to_led(vect &x, std::function<vect(index1)> enties_cycle, const std::map<index, index1> &cycle_index)
{
	...
	std::function<vect(vect &&, vect &&)> adder = [this](vect &&x, vect &&y)
	{ return this->add(std::move(x), std::move(y)); };
...
public:
	matrix_array(unsigned rank, Modules::ModuleOp<matrix_index, R> *moop, Algebra::IOSet<typename matrix<R>::vect> *ioop) : matrix<R>(moop, ioop, &this->data), data(rank) {}

	matrix_array(unsigned rank, Modules::ModuleOp<matrix_index, R> *moop) : matrix<R>(moop, moop, &this->data), data(rank) {}

	matrix_array(stR, unsigned rank, Modules::ModuleOp<matrix_index, R> *moop) : matrix<R>(moop, moop, &this->data), data(rank) {}

	matrix_array(Modules::ModuleOp<matrix_index, R> *moop) : matrix<R>(moop, moop, &this->data) {}
...
private:
	matrix_index symplify_to_led0(vect &x, matrix_index start)
	{
		...
		if (ModOper->ROper->invertible(ModOper->component(start, x)))
			return start;
		else
			return symplify_to_led(x, start + 1);
	}
	...
	matrix_index symplify_to_led(vect &x, matrix_index start, vect &homotopy)
	{
		...
		if (ModOper->ringOper->invertible(ModOper->component(start, x)))
			return start;
		else
			return symplify_to_led(x, start + 1, homotopy);
	}
...
public:
	...
	{
		string res = "";
		std::function<void(entry &)> ous = [&res](entry &et)
		{ res += std::to_string(et.cycle) + "\t<-\t" + std::to_string(et.tag) + "\n"; };
		data->update_all(ous);
		return res;
	}
...
public:
	...
	matrix_index symplify_to_led0(vect &x)
	{
		auto &entries = datas.data;
		std::function<vect(vect &&, vect &&)> adder = [this](vect &&x, vect &&y)
		{ return this->ModOper->add(std::move(x), std::move(y)); };
...
void matrix<R>::direct_sum(matrix<R> const &Y, unsigned tar_rank)
{
	...
	std::function<void(vect &, matrix_index)> row_summer = [&Y, tar_rank](vect &ro, matrix_index i)
...
{
	std::function<bool(matrix_index)> tule = [&to_del](matrix_index n)
